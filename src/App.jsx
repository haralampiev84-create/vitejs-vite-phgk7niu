...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      if (saved.totalReceived) setTotalReceived(saved.totalReceived);
    ...
    if (day >= 20 && day <= 28) {
      try {
        ...
        if (!salaryCheck.received && salaryCheck.lastAskedDay !== todayStr()) {
  ...
  useEffect(() => {
    localStorage.setItem(STORAGE_KEY, JSON.stringify({ totalReceived, expenses, extraIncomes }));
  }, [totalReceived, expenses, extraIncomes]);

  const totalExtra = extraIncomes.reduce((s, e) => s + e.amount, 0);
  const totalIncome = totalReceived + totalExtra;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0f0f13", fontFamily: "'DM Sans', 'Segoe UI', sans-serif", color: "#e8e4df", paddingBottom: 80 }}>
      <div style={{ background: "linear-gradient(135deg, #1a1a24 0%, #12121a 100%)", borderBottom: "1px solid #2a2a38", padding: "20px 20px 0", position: "sticky", top: 0, zIndex: 10 }}>
        ...
        <div style={{ display: "flex", gap: 0, borderTop: "1px solid #2a2a38" }}>
          {[["dashboard","📊 Обзор"], ["history","📋 История"], ["add","+ Разход"]].map(([v, label]) => (
            <button key={v} onClick={() => setView(v)} style={{ flex: 1, padding: "12px 0", background: "none", border: "none", borderBottom: view === v ? "2px solid #f97316" : "2px solid transparent", color: view === v ? "#f97316" : "#6b6b80", fontWeight: view === v ? 700 : 400, fontSize: 13, cursor: "pointer", transition: "all .2s" }}>{label}</button>
          ))}
        </div>
      </div>
        ...
        {showSalaryPrompt && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }}>
            <div style={{ background: "#1a1a24", borderRadius: 20, padding: 28, width: 300, border: "1px solid #22c55e" }}>
              ...
              <div style={{ fontSize: 13, color: "#6b6b80", textAlign: "center", marginBottom: 20 }}>Ако да, въведи сумата и тя ще се добави към баланса ти.</div>
              <input value={salaryInput} onChange={e => setSalaryInput(e.target.value)} placeholder="Сума (€)" type="text" inputMode="decimal"
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={handleSalaryNo} style={{ flex: 1, padding: "13px 0", borderRadius: 12, background: "#12121a", border: "1px solid #2a2a38", color: "#6b6b80", cursor: "pointer", fontSize: 14, fontWeight: 600 }}>Още не</button>
                <button onClick={handleSalaryYes} disabled={!salaryInput} style={{ flex: 1, padding: "13px 0", borderRadius: 12, background: salaryInput ? "#22c55e" : "#2a2a38", border: "none", color: "#fff", cursor: salaryInput ? "pointer" : "not-allowed", fontSize: 14, fontWeight: 700 }}>Да! ✓</button>
              </div>
            </div>
          </div>
        )}
        ...
        {view === "dashboard" && (
          <div>
            <div style={{ background: "linear-gradient(135deg, #1e1e2e, #16161f)", borderRadius: 20, padding: 24, marginBottom: 16, border: "1px solid #2a2a38", boxShadow: flash ? "0 0 0 2px #22c55e" : "none", transition: "box-shadow .3s" }}>
              <div style={{ fontSize: 12, color: "#6b6b80", marginBottom: 6, letterSpacing: 1 }}>ТЕКУЩ БАЛАНС</div>
              <div style={{ fontSize: 42, fontWeight: 800, color: balanceColor, lineHeight: 1 }}>{fmt(balance)} <span style={{ fontSize: 18, fontWeight: 400 }}>€</span></div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
              {[
                ["Общо приходи", fmt(totalIncome), "#22c55e"],
                ["Общо разходи", fmt(totalSpent), "#ef4444"],
                ["Допълнителни", fmt(totalExtra), "#3b82f6"],
                ["Заплати", fmt(totalReceived), "#f97316"],
              ].map(([label, val, color]) => (
          ...
          <div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                {CATEGORIES.map(c => (
                  <button key={c.id} onClick={() => setCategory(c.id)} style={{ padding: "11px 12px", borderRadius: 12, fontSize: 13, textAlign: "left", background: category === c.id ? c.color + "22" : "#1a1a24", border: category === c.id ? `1.5px solid ${c.color}` : "1px solid #2a2a38", color: category === c.id ? c.color : "#a0a0b8", cursor: "pointer", fontWeight: category === c.id ? 600 : 400, transition: "all .15s" }}>{c.label}</button>
                ))}
              </div>
            </div>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 4, marginBottom: 16 }}>
              {[{ id: "all", label: "Всички" }, ...CATEGORIES].map(c => (
                <button key={c.id} onClick={() => setFilterCat(c.id)} style={{ padding: "7px 14px", borderRadius: 99, fontSize: 12, whiteSpace: "nowrap", background: filterCat === c.id ? "#f97316" : "#1a1a24", border: filterCat === c.id ? "none" : "1px solid #2a2a38", color: filterCat === c.id ? "#fff" : "#a0a0b8", cursor: "pointer", transition: "all .15s" }}>{c.label || c.id}</button>
              ))}
            </div>
            {filtered.length === 0 && (
            ...
            {filtered.map(e => {
              ...
              return (
                <div key={e.id} style={{ background: "#1a1a24", borderRadius: 14, padding: "14px 16px", marginBottom: 10, border: "1px solid #2a2a38", display: "flex", alignItems: "center", gap: 14 }}>
                  <div style={{ width: 40, height: 40, borderRadius: 12, flexShrink: 0, background: cat.color + "22", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>{cat.label.split(" ")[0]}</div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14, fontWeight: 600, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{e.note}</div>
                    <div style={{ fontSize: 11, color: "#6b6b80", marginTop: 2 }}>{d.toLocaleDateString("de-DE")} · {d.toLocaleTimeString("de-DE", { hour: "2-digit", minute: "2-digit" })}</div>
                  </div>
